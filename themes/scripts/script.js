(function () {
    ...
    function build(slideNumber) {
        var build = function(selector, classToAdd){
            var itemsToBuild = document.querySelectorAll(getSlideHash(slideNumber) + selector);
            if (itemsToBuild.length > 0) {
                var item = itemsToBuild[0];
                item.classList.add(classToAdd);
                return true;
            }
            return false;
        };
        if (build(" .build:not(.built)", "built")) {
            return true;
        }
        if (build(" .build-list li:not(.built)", "built")) {
            return true;
        }
        if (build(" pre.build-code mark:not(.important)", "important")) {
            return true;
        }
        return false;
    }
